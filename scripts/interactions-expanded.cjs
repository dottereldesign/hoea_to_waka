...
(async()=>{const server=serve(8771),browser=await chromium.launch({channel:'chrome',headless:true});const errors=[];try{
  ...
  const dialog=p.locator('.ds-picker');assert.equal(await dialog.locator('.ds-choice:visible').count(),26);
  const first=await p.evaluate(()=>window.HoeaDesignStudio.purposeLayouts.find(c=>c.type==='header'));
  await dialog.getByRole('searchbox',{name:'Search designs'}).fill(first.name);
  await dialog.getByRole('button',{name:'Favourite '+first.name,exact:true}).click();
  await dialog.locator('.ds-choice[data-layout-id="'+first.id+'"]').click();assert.equal(await p.locator('.ds-type-header').getAttribute('data-design'),String(first.id));
  await dialog.getByRole('searchbox',{name:'Search designs'}).fill('');await dialog.getByRole('button',{name:'Favourites',exact:true}).click();assert.equal(await dialog.locator('.ds-choice:visible').count(),1);
  await p.screenshot({path:'tmp/design-studio/expanded/picker-mobile.png'});
  await p.keyboard.press('Escape');await p.reload();await p.waitForFunction(()=>window.HoeaDesignStudio?.version>=2);assert.equal(await p.locator('.ds-type-header').getAttribute('data-design'),String(first.id));
  await p.getByRole('button',{name:'Open design studio',exact:true}).click();await dialog.getByRole('button',{name:'Header Sections 25 layouts',exact:true}).click();await dialog.locator('.ds-category-targets button').click();assert.equal(await dialog.locator('.ds-category-context h3').innerText(),'Header Sections');await p.keyboard.press('Escape');
  ...
  await p.getByRole('button',{name:'Change form design',exact:true}).click();assert.equal(await dialog.locator('.ds-choice:visible').count(),26);
  await dialog.getByRole('searchbox').fill('Afterglow');await dialog.locator('.ds-choice:visible').click();
  ...
  assert.deepEqual(errors,[]);console.log('PASS: categorized choices, search, favourites, persistence, compatible full-page API, all 51 form states, intercepted mailto, quick contact, Original reset, design=50, all 45 motion-enabled mounts.');
