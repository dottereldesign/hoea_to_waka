...
(async()=>{
  ...
  try{
    ...
    assert.match(copied,/small group of six adults/);assert.match(copied,/Output: four separate high-resolution 16:9 images/);assert.match(copied,/Cobalt Atelier/);
    ...
    assert.ok(prompts.every(text=>text.includes('Generate FOUR separate images')&&text.includes('16 variations in total')&&text.includes('Image 4 — experimental interpretations')&&text.includes('not a duplicate, recolour, filter or minor edit')));
    assert.ok(prompts.every(text=>!text.includes('one finished')));
