...
(async()=>{const server=serve(8789),browser=await chromium.launch({channel:'chrome',headless:true});try{
  ...
  await page.getByRole('button',{name:'Open colours and typography',exact:true}).click();await page.getByRole('tab',{name:'Typography',exact:true}).click();assert.equal(await page.locator('#ap-font-display option').count(),20);await page.locator('#ap-font-display').selectOption('lora');await page.keyboard.press('Escape');
  await page.evaluate(()=>window.HoeaDesignStudio.choose('about/:1:header',126));await page.reload();await ready();assert.equal(await page.locator('.ds-type-header').getAttribute('data-design'),'126');assert.equal(await page.evaluate(()=>window.HoeaAppearance.getState().roles.display),'lora');
